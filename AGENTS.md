...
- `application/src/MainWindow.cpp`
  - Main wxFrame, StreamKit controls, users table, pop-out logs window.
...
- `application/vcpkg.json`
  - Dependencies: `curl`, `nlohmann-json`, `wxwidgets`.
...
4. The evaluated JavaScript scrapes real StreamKit user rows:
...
5. Results flow back through callbacks to the UI thread via `CallAfter`.
